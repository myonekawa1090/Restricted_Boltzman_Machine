...
class NeuralNet:
    ...
    def train(self, data_org, target_org, epochs=15, learning_rate=0.1,batch_size=100, momentum_rate=0.5 , monitor=2000):
        ...
        for epoch in range(epochs):
            ...
            for i in xrange(0 , N , batch_size):
                ...
                y = sigmoid(np.dot(z1 , W2) + out_bias)
                # Delta
                #out_delta = d - y
                ...
                h1_delta = np.dot(W2 , out_delta.T).T * dsigmoid(z1) / batch_size
                grad_W1 = np.dot(np.atleast_2d(h1_delta).T , np.atleast_2d(x))
                in_delta = dsigmoid(x) * (np.dot(h1_delta , W1.T)) / batch_size
                # gradients
...
if __name__ == "__main__":
    ...
    nn = NeuralNet(2,2,2)
    nn.train(inputs , targets , epochs=50000, learning_rate=0.1, batch_size=1, momentum_rate=0.5)
